    ...
    <div className={`group relative transition-all duration-300 ${rotation}`}>
      ...
      >
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <div className="absolute bottom-2 left-2 right-2 flex gap-1">
              <Button
                size="sm"
                className="flex-1 h-7 px-2 text-xs font-handwritten border border-zinc-900 dark:border-white bg-zinc-50 dark:bg-zinc-800 text-zinc-900 dark:text-white hover:bg-white dark:hover:bg-zinc-700"
                onClick={(e) => {
                  e.stopPropagation();
                  onView(id);
                }}
              ...
              {price && (
                <>
                  <Button
                    size="sm"
                    className="h-7 px-2 text-xs font-handwritten border border-zinc-900 dark:border-white bg-green-500 text-white hover:bg-green-600"
                    onClick={(e) => {
                      e.stopPropagation();
                      handleAddToCart();
                    }}
                  ...
                  <Button
                    size="sm"
                    className="h-7 px-2 text-xs font-handwritten border border-zinc-900 dark:border-white bg-amber-400 text-zinc-900 hover:bg-amber-300"
                    onClick={(e) => {
                      e.stopPropagation();
                      if (price) onPurchase({
                        id,
        ...
        <div className="p-3 space-y-2">
          <div>
            <h3 className="text-sm font-bold font-handwritten text-zinc-900 dark:text-white group-hover:text-amber-500 transition-colors line-clamp-1">
              {title}
            </h3>
          </div>
          ...
          <div className="flex justify-between items-center">
            {price && (
              <span className="text-base font-bold font-handwritten text-amber-600 dark:text-amber-400">₹{price}</span>
            )}
          </div>
        </div>
      </Card>
