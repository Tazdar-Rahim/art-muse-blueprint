...
const Navigation = ({ activeSection, onNavigate }: NavigationProps) => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-md border-b-2 border-zinc-900 dark:border-white mobile-shadow shadow-zinc-900 dark:shadow-white">
      <div className="container mx-auto mobile-padding">
        <div className="flex items-center justify-between h-14 sm:h-16">
          {/* Creative Logo - Mobile Optimized */}
          <div 
            className="flex items-center gap-2 sm:gap-3 cursor-pointer group relative touch-target" 
          ...
          >
            ...
            <div className="relative hidden xs:block">
              <h2 className="text-lg sm:text-xl font-bold font-handwritten text-zinc-900 dark:text-white rotate-[-1deg]">Farhana</h2>
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            <SheetTrigger asChild>
              <Button 
                variant="ghost" 
                size="sm" 
                className="font-handwritten border-2 border-zinc-900 dark:border-white bg-zinc-50 dark:bg-zinc-800 hover:bg-white dark:hover:bg-zinc-700 mobile-shadow shadow-zinc-900 dark:shadow-white mobile-hover-shadow transition-all duration-200 rounded-lg touch-target touch-interaction"
              >
                      ...
                      <div key={item.id} className={`relative ${rotation}`}>
                        <Button
                          ...
                          className={`
                            ...
                            ${isActive 
                              ? "bg-amber-400 text-zinc-900 shadow-[3px_3px_0px_0px] sm:shadow-[4px_4px_0px_0px]" 
